    ...
    internal class CallSiteFactory
    {
        ...
        private void Populate()
        {
            foreach (var descriptor in _descriptors)
            {
                ...
                if (serviceTypeInfo.IsGenericTypeDefinition)
                {
                    ...
                    {
                        throw new ArgumentException(string.Format(Resources.OpenGenericServiceRequiresOpenGenericImplementation,descriptor.ServiceType),"descriptors");
                    }

                    if (implementationTypeInfo.IsAbstract || implementationTypeInfo.IsInterface)
                    {
                        throw new ArgumentException(string.Format(Resources.TypeCannotBeActivated,descriptor.ImplementationType, descriptor.ServiceType));
                    }
                }
        ...
        private ServiceCallSite TryCreateExact(ServiceDescriptor descriptor, Type serviceType, CallSiteChain callSiteChain, int slot)
        {
            if (serviceType == descriptor.ServiceType)
            {
                ...
                return new InterceptionCallSite(serviceType, descriptor.ImplementationType, _proxyFactory, callSite);
            }
            ...
            CallSiteChain callSiteChain)
        {
            try
            {
                        ...
                        else
                        {
                            ...
                            if (!bestConstructorParameterTypes.IsSupersetOf(parameters.Select(p => p.ParameterType)))
                            {
                                ...
                                var message = string.Join(
                                    Environment.NewLine,
                                    string.Format(Resources.AmbiguousConstructorException,implementationType),
                                    bestConstructor,
                                    constructors[i]);
                ...
                if (bestConstructor == null)
                {
                    throw new InvalidOperationException(string.Format(Resources.UnableToActivateTypeException,implementationType));
                }
                else
                {
            ...
            bool throwIfCallSiteNotFound)
        {
            ...
            for (var index = 0; index < parameters.Length; index++)
            {
                ...
                if (callSite == null)
                {
                    ...
                    {
                        throw new InvalidOperationException(string.Format(Resources.CannotResolveService, parameterType, implementationType));
                    }
