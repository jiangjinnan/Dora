...
namespace Microsoft.Extensions.DependencyInjection.ServiceLookup
{
    internal sealed class CallSiteRuntimeResolver : CallSiteVisitor<RuntimeResolverContext, object>
    {
        ...
        protected override object VisitInterception(InterceptionCallSite interceptionCallSite, RuntimeResolverContext argument)
        {
            if (interceptionCallSite.CanIntercept)
            {
                if (interceptionCallSite.ServiceType.IsInterface)
                {
                    var target = VisitCallSite(interceptionCallSite.Target, argument);
                    return interceptionCallSite.ProxyFactory.Wrap(interceptionCallSite.ServiceType, target);
                }
                return interceptionCallSite.ProxyFactory.Create(interceptionCallSite.ServiceType, argument.Scope);
            }
            return VisitCallSite(interceptionCallSite.Target, argument);
        }
    }
