...
{
    public class VirtualMethodProxyGeneratorFixture_General
    {
        [Fact]
        public void InvokeAsVoid_CaptureArguments()
        {
            ...
            Assert.Equal(typeof(Foobar).GetMethod(nameof(Foobar.InvokeAsVoid)), FakeInterceptor.InvocationContext.Method);
        ...
        public void InvokeAsResult_CaptureArguments()
        {
            ...
            Assert.Equal(2, FakeInterceptor.InvocationContext.Arguments[1]);
            Assert.Same(proxy, FakeInterceptor.InvocationContext.Target);
            Assert.Equal(typeof(Foobar).GetMethod(nameof(Foobar.InvokeAsResult)), FakeInterceptor.InvocationContext.Method);
            Assert.True(FakeInterceptor.Invoked);
            Assert.True(Foobar.Invoked);
        ...
        public async void InvokeAsTask_CaptureArguments()
        {
            ...
            Assert.Equal(2, FakeInterceptor.InvocationContext.Arguments[1]);
            Assert.Same(proxy, FakeInterceptor.InvocationContext.Target);
            Assert.Equal(typeof(Foobar).GetMethod(nameof(Foobar.InvokeAsTask)), FakeInterceptor.InvocationContext.Method);
            Assert.True(FakeInterceptor.Invoked);
            Assert.True(Foobar.Invoked);
        }
        ...
        public async void InvokeAsTaskOfResult_CaptureArguments()
        {
            ...
            Assert.Equal(2, FakeInterceptor.InvocationContext.Arguments[1]);
            Assert.Same(proxy, FakeInterceptor.InvocationContext.Target);
            Assert.Equal(typeof(Foobar).GetMethod(nameof(Foobar.InvokeAsTaskOfResult)), FakeInterceptor.InvocationContext.Method);
            Assert.True(FakeInterceptor.Invoked);
            Assert.True(Foobar.Invoked);
            Assert.Equal(3, result);
        }
        ...
        public async void InvokeAsValueTask_CaptureArguments()
        {
            ...
            Assert.Equal(2, FakeInterceptor.InvocationContext.Arguments[1]);
            Assert.Same(proxy, FakeInterceptor.InvocationContext.Target);
            Assert.Equal(typeof(Foobar).GetMethod(nameof(Foobar.InvokeAsValueTask)), FakeInterceptor.InvocationContext.Method);
            Assert.True(FakeInterceptor.Invoked);
            Assert.True(Foobar.Invoked);
        }
        ...
        public async void InvokeAsValueTaskOfResult_CaptureArguments()
        {
            ...
            Assert.Null(FakeInterceptor.InvocationContext.Arguments);
            Assert.Same(proxy, FakeInterceptor.InvocationContext.Target);
            Assert.Equal(typeof(Foobar).GetMethod(nameof(Foobar.InvokeAsValueTaskOfResult)), FakeInterceptor.InvocationContext.Method);
            Assert.True(FakeInterceptor.Invoked);
            Assert.True(Foobar.Invoked);
            Assert.Equal(3, result);
        }
        ...
        public class Foobar
        {
            ...
            public virtual int InvokeAsResult(int x, int y)
            {
                Invoked = true;
                return x + y;
            }

            public virtual Task InvokeAsTask(int x, int y)
            {
                Invoked = true;
                return Task.Delay(10);
            }

            public virtual async Task<int> InvokeAsTaskOfResult(int x, int y)
            {
                Invoked = true;
                await Task.Delay(10);
                return x + y;
            ...
            public virtual async ValueTask InvokeAsValueTask(int x, int y)
            {
                Invoked = true;
                await Task.Delay(10);
            }

            public virtual async ValueTask<int> InvokeAsValueTaskOfResult(int x, int y)
            {
                Invoked = true;
                await Task.Delay(10);
                return x + y;
            ...
            public virtual void InvokeAsVoid(int x, int y)
            {
                Invoked = true;
            }
        }
